// Tabela de NCMs agropecuários com informações tributárias (LC 214/2025)
const NCM_AGRO: Record<
  string,
  { descricao: string; cesta: "zero" | "reduzida" | "normal"; reducao: number; capitulo: string; is: boolean }
> = {
  ...
  "2302": { descricao: "Sêmeas, farelos e resíduos de cereais", cesta: "reduzida", reducao: 60, capitulo: "Rações", is: false },
  "2303": { descricao: "Resíduos de amido, polpas de beterraba, bagaço de cana", cesta: "reduzida", reducao: 60, capitulo: "Rações", is: false },
  "2304": { descricao: "Tortas e resíduos de extração do óleo de soja (farelo)", cesta: "reduzida", reducao: 60, capitulo: "Rações", is: false },
...
export function consultarNcm(ncm: string) {
  ...
  return {
    ...
    capituloNumero: ncm4.substring(0, 2),
    posicao: ncm4,
    cestaTributaria: info.cesta,
    reducaoAliquota: `${info.reducao}%`,
    impostoSeletivo: info.is,
    observacao:
      info.cesta === "zero"
...
export function listarNcms(filtro?: { capitulo?: string; cesta?: string }) {
  return Object.entries(NCM_AGRO)
    .filter(([, info]) => {
      if (filtro?.capitulo && info.capitulo !== filtro.capitulo) return false;
      if (filtro?.cesta && info.cesta !== filtro.cesta) return false;
      return true;
    })
    .map(([ncm, info]) => ({
      ncm,
